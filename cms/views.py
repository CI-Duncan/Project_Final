...
def note_create(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Automatically set the author as the logged-in user (carer)
            ...
            try:
                ...
                # Redirect to the newly created note's content page
                return redirect('cms:note_content', pk=note.pk)
            except Exception as e:
                logger.error(f"Failed to save note: {e}")
                messages.error(
                    request, 'Failed to save note. Please try again.')
        else:
    ...
    return render(request, 'cms/note_create.html',
...
def note_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Redirect to the note content page after successful edit
            return redirect('cms:note_content', pk=note.pk)
        else:
...
def note_delete(request, pk):
    ...
    if hasattr(request.user, 'client') and request.user.client.id == pk:
        ...
        return render(request, 'cms/error.html',
                      {'error_message': 'Clients cannot delete their own notes.'})
        ...
        messages.error(
            request,
            'Carers cannot delete notes of clients they do not manage. Please contact an administrator if this note needs to be removed.')
...
def client_list(request):
    if request.method == "GET":
        ...
        if query:
            # Filter clients based on the query and check if the user is the
            # client or a carer
...
def client_edit(request, pk):
    ...
    if hasattr(request.user, 'client') and request.user.client.id == pk:
        # The user is the client themselves, deny edit access, only view
        return render(request, 'cms/error.html',
                      {'error_message': 'You do not have permission to edit this client.'})
    elif hasattr(request.user, 'carer') and request.user.carer.clients.filter(pk=pk).exists():
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                form.save()
                messages.add_message(
                    request, messages.SUCCESS, f'{
                        client.first_name} {
                        client.last_name} has been successfully updated.')
                return redirect('cms:client_list')
        else:
...
def client_delete(request, pk):
    ...
    if hasattr(
            request.user,
            'carer') and request.user.carer.clients.filter(
            pk=pk).exists():
        if request.method == 'POST':
            ...
            return render(request,
                          'cms/client_confirm_delete.html',
                          {'client': client})
    else:
        # The user is not a carer associated with the client, deny deletion
        return render(request, 'cms/error.html',
                      {'error_message': 'You do not have permission to delete this client.'})
...
def client_detail(request, pk):
    ...
    if request.user.is_authenticated:
        if hasattr(request.user, 'client') and request.user.client.id == pk:
            ...
            # The user is neither the specific client nor a carer associated
            # with the client
            return render(request, 'cms/error.html',
                          {'error_message': 'You do not have permission to view this client.'})
